...
- **Интеграция с Telegram:** Telegram-бот, реализованный с использованием Aiogram, отправляет уведомления пользователям:
  - **Уведомление об оформлении заказа:** При размещении заказа отправляется сообщение с фотографией (если она доступна) и информацией о заказе.
...
│       ├── views.py        # Основные представления: каталог, корзина, оформление заказа, профиль, страница админа
│       ├── forms.py        # Формы для регистрации пользователей и оформления заказа
│       ├── urls.py         # URL-адреса приложения (если используются)
│       └── templates/
│           └── shop/
